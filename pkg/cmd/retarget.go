...
func Retarget(ctx *clicontext.CommandContext, spaceName, targetCluster string) error {
	...
	space, err := client.GetSpace(hostClusterClient, hostClusterConfig.KubeSawNamespace, spaceName)
	if err != nil {
		return err
	}
	...
	userSignup, err := client.GetUserSignup(hostClusterClient, hostClusterConfig.KubeSawNamespace, creator)
	if err != nil {
		return err
	}
