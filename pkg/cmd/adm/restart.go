...
func restart(ctx *clicontext.CommandContext, clusterName string, deployments ...string) error {
	...
	if !ctx.AskForConfirmation(
		ioutils.WithMessagef("restart the deployment '%s' in namespace '%s'", deploymentName, cfg.KubeSawNamespace)) {
		return nil
	}
...
func restartHostOperator(ctx *clicontext.CommandContext, hostClient runtimeclient.Client, hostConfig configuration.ClusterConfig) error {
	...
	if err := hostClient.List(context.TODO(), deployments,
		runtimeclient.InNamespace(hostConfig.KubeSawNamespace),
	...
	if len(deployments.Items) != 1 {
		return fmt.Errorf("there should be a single deployment matching the label olm.owner.namespace=toolchain-host-operator in %s ns, but %d was found. "+
			"It's not possible to restart the Host Operator deployment", hostConfig.KubeSawNamespace, len(deployments.Items))
	}
...
func printExistingDeployments(term ioutils.Terminal, cl runtimeclient.Client, cfg configuration.ClusterConfig) error {
	deployments := &appsv1.DeploymentList{}
	if err := cl.List(context.TODO(), deployments, runtimeclient.InNamespace(cfg.KubeSawNamespace)); err != nil {
		return err
	}
	deploymentList := "\n"
	...
	term.PrintContextSeparatorWithBodyf(deploymentList, "Existing deployments in %s namespace", cfg.KubeSawNamespace)
	return nil
}
